...
test('e2e completo', async ({ page }) => {
  ...
  await test.step(`Teste de login passou com sucesso após ${((Date.now() - loginStart) / 1000).toFixed(2)}s`, () => {});

  const androidStart = Date.now();
  await runAndroidBuildTest('e2e : android');
  await test.step(`Teste de build Android passou com sucesso após ${((Date.now() - androidStart) / 1000).toFixed(2)}s`, () => {});
