...
const DEFAULT_TRANSITION_DURATION = 45; // seconds
...
export const estimateWorkoutDuration = (
  ...
  const totalSeconds = exercises.reduce((acc, ex) => {
    ...
    // Parse reps from string if needed, or use default
    // WorkoutTemplateExercise has 'sets' as string like "3x12"
    let reps = 10;
    ...
    // Parse rest from string like "60s" or "60,90,120" or "1:30"
    let restSeconds = 60;
    if (ex.rest.includes(',')) {
      ...
      // Sum the first (numSets - 1) rests
      const totalRest = rests.slice(0, ex.numSets - 1).reduce((a, b) => a + b, 0);
      // Average it for estimateExerciseDuration which expects a single value and multiplies it
